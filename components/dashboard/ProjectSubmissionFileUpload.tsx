...
import { deleteFile, fetchAllFiles } from "@/data/project-submission-file";
import { extractAndSaveProjectSubmissionMarks } from "@/data/student";
...
export default function ProjectSubmissionFilesUpload({
  refreshResults,
  ...
  const extractAndSaveHandle = async (id: string) => {
    ...
    try {
      const data = await extractAndSaveProjectSubmissionMarks(id);
      if (!data) {
